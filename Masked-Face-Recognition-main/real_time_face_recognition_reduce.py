...
def stream(database_num, pb_path, node_dict, ref_dir, camera_source=0, resolution="480", to_write=False, save_dir=None):
    # ----var
    frame_count = 0
    FPS = "loading"
    ...
    # ----Get an image
    while (cap.isOpened()):
        # 统计人脸库的照片数量，如果有人新上传了自己的图片，那么就要重启系统去加载这个人脸库
        count = 0
            ...
            if len(bboxes) > 0:
                for num, bbox in enumerate(bboxes):
                    ...
                    if name!="strange man":
                        ...
                        if os.path.exists('./校内人员数量.txt')==False:
                            with open('./校内人员数量.txt', 'w') as f:
                                f.write('0')
                        if os.path.exists('./校外人员数量.txt')==False:
                            with open('./校外人员数量.txt', 'w') as f:
                                f.write('0')
                        ...
                        with open(file_name, 'r', encoding='utf-8') as f:
                            ...
                            if lines == []:
                                ...
                                last_line_state = lines[-1].split(' ')[0]
                                if last_line_state!='出去':
                                    out_to_in=1

                        if out_to_in==1:
                            with open("./校内人员数量.txt", 'r', encoding='utf-8') as f:
                                line_human=f.readline()
                                human_number=int(line_human)-1
                            with open("./校内人员数量.txt", 'w', encoding='utf-8') as f:
                                f.write(str(human_number))
                            with open(file_name, 'a', encoding='utf-8') as f:
                                f.write("出去 " + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '\n')
...
if __name__ == "__main__":
    camera_source = "http://admin:admin@10.242.136.207:8081"  # can be also the path of a clip
    ...
    while 1:
        ...
        for root, dirs, files in os.walk("./database/"):
            for file in files:
                ...
                if ext == '.jpg':
                    count = count + 1
        stream(count, pb_path, node_dict, ref_dir, camera_source=camera_source, resolution="720", to_write=False, save_dir=None)
